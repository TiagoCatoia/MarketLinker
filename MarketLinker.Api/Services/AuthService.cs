...
public class AuthService : IAuthService
{
    ...
    public string GenerateAccessToken(string userId)
    {
        ...
        var key = new SymmetricSecurityKey(
            Encoding.UTF8.GetBytes(_config[jwtKey] ??
                                   throw new InvalidOperationException($"Config '{jwtKey}' missing")));
    ...
    public async Task<TokenResponseDto> RefreshTokenAsync(string refreshToken, string deviceName,
    ...
    public async Task<TokenResponseDto> GenerateAndSaveTokensAsync(Guid userId, string deviceName,
        CancellationToken cancellationToken)
    {
